...
const GridStatusCard = ({ gridStatus, grid, gridChartData }) => {
  const formattedGrid = formatPower(Math.abs(grid));
  ...
  return (
    <div className="card border-2 border-base-300 p-6 flex flex-col gap-2">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="inline-flex items-center gap-1 ">
          <div className="flex items-end gap-2">
            ...
            <span className="mb-1">{formattedGrid.unit}</span>
          </div>
        </span>
      </div>
      ...
      <div className="text-xs  mt-1">Energy flow to grid (export/import)</div>
    </div>
  );
};
