...
const Battery = ({
  ...
  conversionValue = "",
  efficiency = 95,
  estimatedTime = "1h 20m", // fallback if not converting
}) => {
  const batteryPercent = Math.min(
    100,
    Math.round((capacity / maxCapacity) * 100)
  );
  const batteryColor =
    batteryPercent > 60
    ...
    <>
      <div className="flex items-center space-x-6">
        ...
        <div className="flex-shrink-0">
          <div className="relative w-16 h-8">
            {/* Battery Body */}
            <div
              className={`absolute left-0 top-0 w-14 h-8 border-2 rounded-md bg-base-200 overflow-hidden ${
                batteryColor.split(" ")[1]
              }`}
            >
              <div
                className={`h-full transition-all duration-700 ${
                  batteryColor.split(" ")[0]
                }`}
            ...
            {/* Battery Head */}
            <div
              className={`absolute right-0 top-2 w-2 h-4 ${
                batteryColor.split(" ")[0]
              } rounded-r`}
            ></div>
          </div>
        ...
        <div>
          ...
          {/* Estimation only appears during conversion */}
          {conversionProgress > 0 && (
          ...
          <span className="flex items-center gap-1 font-semibold">
            <span
              className={`inline-block w-3 h-3 rounded-full ${
                smartMeterOnline ? "bg-green-400" : "bg-gray-300"
              }`}
            ></span>
            <span
              className={`text-sm ${
                smartMeterOnline ? "text-success" : "text-base-content/40"
              }`}
            >
      ...
      <div className={`mt-4 w-full ${conversionProgress > 0 ? "" : "hidden"}`}>
        ...
        <div className="flex justify-between text-xs mt-2 text-base-content/60">
          <span>
            Efficiency: <span className="font-mono">{efficiency}%</span>
          </span>
